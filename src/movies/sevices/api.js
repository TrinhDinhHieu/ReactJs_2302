...
//UpComming Movies
const UpCommingMovie = async (startDate, endDate,page = 1) => {
...
//Sarch Movies
const searchMovieByName = async (keywork,page=1) => {
  const url = `https://api.themoviedb.org/3/search/movie?query=${keywork}&api_key=cfe422613b250f702980a3bbf9e90716&page=${page}`;
  const response = await axios.get(url);
  return (await response.status) === 200 ? await response.data : {};
};
